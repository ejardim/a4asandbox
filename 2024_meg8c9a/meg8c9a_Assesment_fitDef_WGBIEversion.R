...
setwd("D:/WorkingGroups/2024_WG/2024WGBIE/a4a/meg8c9a/data")

load("../out/meg8c9ainputs.RData")

load("../out/meg8c9aIndices.RData")


SavePlot<-function(plotname,width=6,height=4){
  file <- file.path(paste0('../out/fitmeg/fitDef/meg8c9a_a4a_',plotname,'.png'))
...
save(stock,tun.sel,fit1,file='../out/fitmeg/fitDef/meg8c9afitDef.Rdata')
...
plot(stk1)
save(stk1,file="../out/fitmeg/fitDef/meg8c9a_stk1.RData")
...
write.csv(sum1,'../out/fitmeg/fitDef/Summary_final.csv',row.names=F)
...
write.csv(sum2,'../out/fitmeg/fitDef/Summary_SAG_1sd.csv', row.names=F)
...
write.csv(sum3,'../out/fitmeg/fitDef/Summary_SAG_2sd.csv', row.names=F)
...
write.csv(sen,'../out/fitmeg/fitDef/Sen.csv',row.names=F)
...
write.csv(fit1@stock.n@.Data,'../out/fitmeg/fitDef/stocknumbers.csv',row.names=T) 
write.csv(fit1@harvest@.Data,'../out/fitmeg/fitDef/harvest.csv',row.names=T) 

#### 
par(mfrow=c(1,1))
kobe <- data.frame(Btrig=725,Fmsy=0.173,B=rev(c(ssb(stk1))),F=rev(c(fbar(stk1))))
with(kobe[1,],plot(B/Btrig,F/Fmsy,xlim=c(0,8),ylim=c(0,5.5),main='meg8c9a'))
rect(-1,-1,8,8,col='yellow',border=NA)
rect(-1,1,1,8,col='red',border=NA)
rect(1,-1,8,1,col='green',border=NA)
